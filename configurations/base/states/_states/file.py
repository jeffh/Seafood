''' TODO: update to develop branch of salt!
...
def _update_file_attributes(name, user=None, group=None, mode=None):
    changes = {}
    prev_user, prev_group = __salt__['file.get_user'](name), __salt__['file.get_group'](name)
    user_changed = (user is not None and user != __salt__['file.get_user'](name))
    group_changed = (group is not None and group != __salt__['file.get_group'](name))
    if user_changed or group_changed:
        user = user or __salt__['file.get_user'](name)
        group = group or __salt__['file.get_group'](name)
        __salt__['file.chown'](name, user, group)
        if user_changed:
            changes['user'] = user
        if group_changed:
            changes['group'] = group
    if mode is not None and mode != __salt__['file.get_mode'](name):
        __salt__['file.set_mode'](name, mode)
        # changes['mode'] = mode # don't report mode, posix doesn't require lchmod
    if changes:
        return {'updated': changes}
    return {}


def symlink(name, target, force=False, makedirs=False, user=None, group=None, mode=None):
    '''
    Create a symlink
    ...
    user
        The user to own the symlink, this defaults to the user salt is running as
        on the minion

    group
        The group ownership set for the symlink, this defaults to the group salt
        is running as on the minion

    mode
        The permissions to set on this symlink, aka 644, 0775, 4664
    '''
    ret = {'name': name,
           'changes': {},
           'result': True,
    ...
    if os.path.islink(name):
        ...
        if not os.readlink(name) == target:
            ...
            result = _update_file_attributes(name, user, group, mode)
            if result:
                ret['changes']['updated'] = name
                ret['comment'] = ' The symlink {0} was updated'.format(name)
                return ret
            # The link looks good!
    ...
    if not os.path.exists(name):
        ...
        result = _update_file_attributes(name, user, name, group)
        ret['comment'] = 'Created new symlink {0}: {1}'.format(name, result)
