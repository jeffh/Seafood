...
def managed(name,
    ...
    if venv_exists and clear:
        ret['changes']['cleared_packages'] = \
            __salt__['pip_alt.freeze'](bin_env=name)
        ret['changes']['old'] = \
    ...
    if requirements:
        before = set(__salt__['pip_alt.freeze'](bin_env=name))
        __salt__['pip_alt.install'](
            requirements=requirements, bin_env=name, runas=runas, cwd=cwd
        )
        after = set(__salt__['pip_alt.freeze'](bin_env=name))
