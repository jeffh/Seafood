# converted chmod / chown to use lchmod and lchown equvilents to support symlinks
'''
...
def get_mode(path):
    ...
    if not os.path.exists(path):
        return -1
    mode = str(oct(os.lstat(path).st_mode)[-4:])
...
def set_mode(path, mode):
    ...
    try:
        getattr(os, 'lchmod', os.chmod)(path, int(mode, 8))
    except Exception:
